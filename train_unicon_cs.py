"""
Training Script with UNICON + Cost-Sensitive Loss for Noisy Labels.

UNICON (Karim et al., CVPR 2022) combined with clinical cost-sensitive
weighting instead of frequency-balanced weights.

    UNICON combines three ideas:
...
CS weights apply to:
    - Warmup CE loss
    - Evaluation functions (val/test CE computation)
    - Note: UNICON's semi-supervised stage uses soft-label CE for the
      supervised loss and MSE for the unsupervised loss; class_weights
      is passed but does not directly affect the soft CE part.
...
Usage:
    python train_unicon_cs.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 20
    python train_unicon_cs.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 10
"""
...
def train_epoch_warmup(model, train_loader, optimizer, device,
    ...
    """Warmup: standard CE on all samples (with cost-sensitive weights)."""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def train_main(args):
    ...
    print("\n" + "=" * 60)
    print("Creating two ResNet18+Projection models for UNICON CS...")
    ...
    # Training
    print("\n" + "=" * 60)
    print(f"Starting UNICON CS training  |  warmup={args.warmup_epochs}  |  "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        phase_tag = "WARMUP" if is_warmup else "UNICON_CS"
        log_entry = {
            'epoch': epoch,
            'phase': phase_tag,
            'n_clean': int(n_clean),
    ...
    test_clinical_risk1 = compute_clinical_risk(test_cm1, args.lambda_risk)
    test_clinical_risk2 = compute_clinical_risk(test_cm2, args.lambda_risk)
    ...
    print(f"\nTest (Net1): Acc={test_acc1:.4f}, BAC={test_bac1:.4f}, "
          f"AUC={test_auc1:.4f}, Risk={test_clinical_risk1:.4f}")
    print(f"Test (Net2): Acc={test_acc2:.4f}, BAC={test_bac2:.4f}, "
          f"AUC={test_auc2:.4f}, Risk={test_clinical_risk2:.4f}")
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  UNICON CS\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with UNICON + Cost-Sensitive Loss for noisy labels '
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  UNICON CS")
    print("=" * 60)
