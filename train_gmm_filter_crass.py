"""
Training Script with GMM Filter + CRASS for Noisy Labels.

GMM Filter + CRASS:
    Extends the standard GMM-based loss filtering with CRASS (Clinical
    Risk-Aware Sample Selection) per-class thresholds.

    The only change from standard GMM Filter (train_filter_loss.py) is in the
    sample selection step: instead of applying a fixed threshold (default 0.5)
    to all classes, CRASS derives mathematically optimal thresholds per class
    based on the clinical risk cost structure (Proposition 1).
...
Strategy:
    1. Warmup phase (default 10 epochs): train on ALL samples.
    2. Filter phase: per-epoch GMM fitting per class to estimate P(clean),
       then apply CRASS risk-aware thresholds for sample selection.

Usage:
    python train_gmm_filter_crass.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 10
    python train_gmm_filter_crass.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 5 --weighted_loss
"""
...
def compute_optimal_thresholds(lambda_risk, c_noise=1.0, c_fp=1.0):
    ...
    where C_y = C_FN = lambda * C_FP if y is the positive (disease) class,
...
# ═══════════════════════════════════════════════════════════════════════════
# GMM-based loss evaluation + CRASS filtering
# ═══════════════════════════════════════════════════════════════════════════
...
def gmm_filter_per_class_crass(losses, labels, all_loss_history, noise_rate,
                                lambda_risk, positive_class=1):
    """Fit a 2-component GMM **per class** and apply CRASS thresholds.

    Identical to standard GMM filter (train_filter_loss.py) except:
        - Instead of a fixed threshold (0.5) for all classes,
          uses CRASS risk-aware thresholds from Proposition 1:
            * Positive class: theta_pos = 1 / (1 + lambda)  (more permissive)
            * Negative class: theta_neg = 0.5  (standard)
...
def train(args):
    ...
    # Training
    print("\n" + "=" * 60)
    print(f"Starting GMM Filter + CRASS training")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Logging
        phase_tag = "WARMUP" if is_warmup else "CRASS-FILTER"
        filter_log_entry = {
            'epoch': epoch,
            'phase': phase_tag,
    ...
    print(f"Method: GMM Filter + CRASS  |  lambda={args.lambda_risk}  |  "
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  GMM Filter + CRASS\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with GMM Filter + CRASS '
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  GMM Filter + CRASS")
