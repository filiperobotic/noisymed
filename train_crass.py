"""
Training Script with CRASS for Noisy Labels.

CRASS (Clinical Risk-Aware Sample Selection):
    Derives mathematically optimal per-class thresholds that minimise the
    expected clinical risk for filtering samples based on GMM probabilities.
    ...
    **Proposition 1 (Optimal per-class threshold):**
    ...
    For lambda = 10 → theta_1* = 0.091 → a positive-class sample is only
    discarded if its probability of being clean is below 9.1%.

    The only hyper-parameter is lambda (cost ratio C_FN / C_FP).
...
Usage:
    python train_crass.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 10
    python train_crass.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 5 --weighted_loss
"""
...
def train(args):
    ...
    # Training ─────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print(f"Starting CRASS training")
    ...
    print(f"Method: CRASS  |  lambda={args.lambda_risk}  |  "
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  CRASS\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with CRASS (Clinical Risk-Aware Sample Selection) '
                    'for noisy labels'
    )
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  CRASS")
