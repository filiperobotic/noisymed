"""
Training Script with DivideMix + Cost-Sensitive Loss for Noisy Labels.

DivideMix (Li et al., ICLR 2020) combined with clinical cost-sensitive
cross-entropy weighting:
    w[positive_class] = lambda_risk   (penalise missing disease / FN)
    w[other_class]    = 1.0           (standard FP cost)

DivideMix combines two main ideas:
...
Cost-sensitive weighting is applied to:
    - The warmup phase (standard CE with CS weights)
    - The evaluation functions
    - The DivideMix semi-supervised labeled loss uses soft targets via
      -torch.mean(torch.sum(mixed_target_x * log_probs_x, dim=1)) which
      does not directly use class_weights.
...
Key hyper-parameters:
    ...
    - lambda_risk:   cost ratio C_FN / C_FP (default 20.0)

Usage:
    python train_dividemix_cs.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 20
    python train_dividemix_cs.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 10
"""
...
def train_epoch_warmup(model, train_loader, optimizer, device,
    ...
    """Warmup epoch: standard CE with cost-sensitive weights on all samples."""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def train_epoch_dividemix(model, labeled_loader, unlabeled_loader,
    ...
    Note: The labeled loss uses soft cross-entropy via
    -torch.mean(torch.sum(mixed_target_x * log_probs_x, dim=1)) which does
    not directly use class_weights. The class_weights parameter is passed
    for interface consistency but is not applied in the semi-supervised loss.
    ...
    Args:
        ...
        T:                Sharpening temperature.
        class_weights:    Optional per-class CE weights (not used in soft CE).
...
def train_main(args):
    ...
    print(f"  w[class {positive_class} (positive)] = {args.lambda_risk:.4f}")
    print(f"  w[class {1 - positive_class} (negative)] = 1.0")

    # Models ───────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("Creating two ResNet18 models for DivideMix + Cost-Sensitive Loss...")
    ...
    # Training ─────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print(f"Starting DivideMix + Cost-Sensitive Loss training  |  "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_warmup:
            # -- Warmup: standard CE with CS weights on all samples ---------
    ...
    test_clinical_risk_net1 = compute_clinical_risk(test_cm1, args.lambda_risk)
    test_clinical_risk_net2 = compute_clinical_risk(test_cm2, args.lambda_risk)
    ...
    print(f"\nTest (Net1): Acc={test_acc1:.4f}, BAC={test_bac1:.4f}, "
          f"AUC={test_auc1:.4f}, Risk={test_clinical_risk_net1:.4f}")
    print(f"Test (Net2): Acc={test_acc2:.4f}, BAC={test_bac2:.4f}, "
          f"AUC={test_auc2:.4f}, Risk={test_clinical_risk_net2:.4f}")
    ...
    results = {
        ...
        'test_net1': {
            ...
            'clinical_risk': test_clinical_risk_net1,
            **test_cm1,
        },
        'test_net2': {
            'accuracy': test_acc2, 'bac': test_bac2,
            'auc_roc': test_auc2, 'auprc': test_auprc2,
            'clinical_risk': test_clinical_risk_net2,
            **test_cm2,
        },
    }
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  DivideMix + Cost-Sensitive Loss\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with DivideMix + Cost-Sensitive Loss for noisy labels '
                    '(Li et al., ICLR 2020)'
    )
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  DivideMix + Cost-Sensitive Loss")
    print("=" * 60)
