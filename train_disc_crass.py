"""
Training Script with DISC + CRASS for Noisy Labels.

DISC + CRASS:
    Extends DISC (Li et al., CVPR 2023) with CRASS (Clinical Risk-Aware
    Sample Selection) per-class thresholds for sample selection.

    The key change from standard DISC (train_disc.py) is in the
    ``disc_select_and_correct`` step: the dynamic threshold used to classify
    samples as clean/ambiguous/discard is made class-aware using CRASS.

    For positive-class samples (disease), the discard threshold is lowered
    (more permissive) and the clean threshold is lowered (easier to classify
    as clean), reducing false negatives.
...
Strategy:
    1. Warmup phase: standard CE on all samples + EMA teacher update.
    2. DISC phase: EMA teacher confidence + memorization + CRASS per-class
       thresholds for dynamic selection and label correction.

Usage:
    python train_disc_crass.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 10
    python train_disc_crass.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 5 --weighted_loss
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# DISC core: confidence estimation + CRASS-aware selection + correction
# ═══════════════════════════════════════════════════════════════════════════
...
def disc_select_and_correct_crass(confidence, memorization, teacher_preds,
    ...
    Args:
        ...
        lambda_risk:    float — CRASS cost ratio C_FN / C_FP.
        positive_class: int — which class index is positive (disease).
    ...
    noisy_labels = np.array(noisy_labels).flatten()
    ...
    n_pos = int((noisy_labels == positive_class).sum())
...
def train_main(args):
    ...
    print(f"  DISC threshold scale for positive class: "
          f"{theta_pos / theta_neg:.4f}")
    ...
    # Model + EMA teacher
    print("\n" + "=" * 60)
    print("Creating ResNet18 (student) + EMA teacher for DISC + CRASS...")
    ...
    # Training
    print("\n" + "=" * 60)
    print(f"Starting DISC + CRASS training")
    ...
    print(f"  warmup={args.warmup_epochs}  |  EMA decay={args.ema_decay}  |  "
          f"threshold_init={args.threshold_init}")
    print("=" * 60)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_warmup:
            ...
            # 3) CRASS-aware dynamic selection + correction
            selected_indices, corrected_labels, disc_info = \
                disc_select_and_correct_crass(
        ...
        phase_tag = "WARMUP" if is_warmup else "DISC+CRASS"
        log_entry = {
            'epoch': epoch,
            'phase': phase_tag,
        ...
        if epoch % args.print_freq == 0 or epoch == 1:
            ...
            print(f"          Clinical Risk (lambda={args.lambda_risk}): "
                  f"{val_risk:.4f}")
            if disc_info is not None:
                print(f"          DISC+CRASS: clean={disc_info['clean']}, "
    ...
    test_risk_t = compute_clinical_risk(test_cm_t, args.lambda_risk)
    ...
    print(f"Method: DISC + CRASS  |  lambda={args.lambda_risk}  |  "
    ...
    print(f"  Clinical Risk: {test_risk_t:.4f}")
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  DISC + CRASS\n")
        ...
        f.write(f"Clinical Risk: {test_risk_t:.4f}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with DISC + CRASS '
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  DISC + CRASS")
