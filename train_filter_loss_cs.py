"""
Training Script with GMM-based Loss Filtering + Cost-Sensitive Loss
for Noisy Labels in Medical Image Classification.

Strategy:
    1. Warmup phase (default 10 epochs): train on ALL samples (no filtering)
       using cost-sensitive weighted cross-entropy.
...
Cost-Sensitive Weighting:
    Instead of frequency-balanced weights, this script uses clinical
    cost-sensitive weights for the cross-entropy loss:
...
Usage:
    python train_filter_loss_cs.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 20
    python train_filter_loss_cs.py --dataset dermamnist_bin --noise_rate 0.4 --warmup_epochs 15 --lambda_risk 10
"""
...
def train(args):
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  —  Loss Filter (GMM per class) + Cost-Sensitive\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with GMM-based loss filtering + cost-sensitive loss for noisy labels'
    )
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  —  Loss Filter (GMM) + Cost-Sensitive")
    print("=" * 60)
