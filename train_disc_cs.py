"""
Training Script with DISC + Cost-Sensitive Loss for Noisy Labels.
...
Cost-Sensitive Loss:
    ...
    - Warmup phase: CS weights used in train_epoch_warmup via
      F.cross_entropy(outputs, labels, weight=class_weights).
    - DISC training phase: train_epoch_disc uses soft cross-entropy
      (-torch.mean(torch.sum(soft_labels * log_probs, dim=1))) which
      does not directly use class_weights. This is fine.
    - Evaluation: CS weights used for computing validation/test loss.

Key hyper-parameters:
    - warmup_epochs:   warmup epochs (standard CE with CS weights)
...
Usage:
    python train_disc_cs.py --dataset pneumoniamnist --noise_rate 0.2 --lambda_risk 20
    python train_disc_cs.py --dataset breastmnist --noise_rate 0.4 --lambda_risk 10
"""
...
def train_epoch_disc(model, corrected_loader, optimizer, device,
    ...
    Note: class_weights are NOT used here because soft CE operates on
    teacher-corrected soft labels, not hard class indices.
    """
    model.train()
    running_loss = 0.0
    correct = 0
...
def train_main(args):
    ...
    print(f"  w[class {positive_class} (positive)] = {args.lambda_risk}")
    print(f"  w[class {1 - positive_class} (negative)] = 1.0")

    # Model + EMA teacher
    print("\n" + "=" * 60)
    print("Creating ResNet18 (student) + EMA teacher for DISC + CS...")
    ...
    # Training
    print("\n" + "=" * 60)
    print(f"Starting DISC + CS training  |  warmup={args.warmup_epochs}  |  "
    ...
    test_clinical_risk_t = compute_clinical_risk(test_cm_t, args.lambda_risk)
    ...
    print(f"  Clinical Risk: {test_clinical_risk_t:.4f}")
    ...
    results = {
        ...
        'test_teacher': {
            ...
            'clinical_risk': test_clinical_risk_t,
            **test_cm_t,
        },
    }
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT RESULTS  --  DISC + Cost-Sensitive\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"Clinical Risk: {test_clinical_risk_t:.4f}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train with DISC + Cost-Sensitive loss for noisy labels '
                    '(Li et al., CVPR 2023)'
    )
    ...
    print("\n" + "=" * 60)
    print("EXPERIMENT CONFIGURATION  --  DISC + Cost-Sensitive")
    print("=" * 60)
